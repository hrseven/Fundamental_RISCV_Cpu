...
module cache_2_random #(
    ...
    bram #(
        ...
        .DATA_WIDTH(TAG_WIDTH + 2) // 最高位为有效位，次高位为脏位，低位为标记位,plus最近使用位
    ) tag_bram_0(
        .clk(clk),
        .raddr(r_index),
        .waddr(w_index),
        .din({w_valid_0, w_dirty_0, tag}),
        .we(tag_we_0),
        .dout({valid_0, dirty_0, r_tag_0})
    );
    bram #(
        .ADDR_WIDTH(INDEX_WIDTH),
        ...
        case(CS)
            ...
            READ: begin
                ...
                if (hit_0||hit_1) begin // 命中
                    ...
                    dirty=(!choose)?dirty_0:dirty_1;
                    r_line=(!choose)?r_line_0:r_line_1;
